...
static const float deltaT = 0.0043f;

ParticleSystem::ParticleSystem() : running(false), moveWall(false), moveXwallLower(true), s(new solver) {}
...
void ParticleSystem::initialize(tempSolver &tp, solverParams &tempParams) {
	...
	cudaCheck(cudaMalloc((void**)&s->elasticTerm, tempParams.numParticles * sizeof(mat3)));  // elastic term 
	cudaCheck(cudaMalloc((void**)&s->Force_internal, tempParams.numParticles * sizeof(float3)));  // internal force term 
	//Diffuse
	...
	cudaCheck(cudaMemset(s->elasticTerm, 0, tempParams.numParticles * sizeof(mat3)));  // elastic 
	cudaCheck(cudaMemset(s->Force_internal, 0, tempParams.numParticles * sizeof(float3)));  // internal 

	// copy initial solver vavlue to device for cuda kernels 
	...
	cudaCheck(cudaMemcpy(s->elasticTerm, &tp.elasticTerm[0], tempParams.numParticles * sizeof(mat3), cudaMemcpyHostToDevice)); // elastic 
	cudaCheck(cudaMemcpy(s->Force_internal, &tp.Force_internal[0], tempParams.numParticles * sizeof(float3), cudaMemcpyHostToDevice)); //internal
	setParams(&tempParams);  // setParameters into cuda kernal for use later
}
...
void ParticleSystem::updateWrapper(solverParams &tempParams) {
	if (running) {/*
		...
		*/
		if(moveWall){
			tempParams.bounds.x -= 0.2f* tempParams.radius;
			tempParams.bounds.x = fmaxf(tempParams.bounds.x, 2 * tempParams.radius ); 
		}
		else
		{
			//tempParams.bounds.x += 0.2f* tempParams.radius;
			//tempParams.bounds.x = fminf(tempParams.bounds.x, tempParams.gridWidth * tempParams.radius ); 
			tempParams.bounds.x =  tempParams.gridWidth * tempParams.radius;
		}
		
		if(moveXwallLower){
			//tempParams.boundsMin.x -= 0.2f* tempParams.radius;
			//tempParams.boundsMin.x = fmaxf(tempParams.boundsMin.x, 0 ); 
			tempParams.boundsMin.x = 0.0f;
		}
		else
		{
			tempParams.boundsMin.x += 0.2f* tempParams.radius;
			tempParams.boundsMin.x = fminf(tempParams.boundsMin.x, (tempParams.gridWidth - 2) * tempParams.radius ); 
		}
